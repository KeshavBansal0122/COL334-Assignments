#!/bin/bash
# Quick test script for Part 2

echo "=================================="
echo "Part 2: Testing Congestion Control"
echo "=================================="
...
if [ -f "${PREF}received_data.txt" ]; then
    ORIG_SIZE=$(wc -c < data.txt)
    RECV_SIZE=$(wc -c < ${PREF}received_data.txt)
    ORIG_MD5=$(md5sum data.txt | awk '{print $1}')
    RECV_MD5=$(md5sum ${PREF}received_data.txt | awk '{print $1}')
    ...
    echo "✗ FAILURE: ${PREF}received_data.txt not found!"
fi
